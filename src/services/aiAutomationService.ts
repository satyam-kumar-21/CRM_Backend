...
import { CompanySalesService } from './companySalesService';
import { AiService } from './aiService';
import { extractTextFromFile, normalizeText } from '../utils/fileParser';
import { decryptSecret, encryptSecret } from '../utils/crypto';
import crypto from 'node:crypto';

export interface JivoWebhookPayload {
  connectionId?: string;
  siteId?: string;
  channelId?: string;
  campaign?: string;
  ...
  chat?: {
    ...
    status?: string;
    lastMessage?: string;
    messages?: Array<{ sender?: string; text?: string; content?: string; role?: string; timestamp?: string }>; 
  };
  message?: {
    content?: string;
    text?: string;
...
const getCompanyId = (companyId: string | Types.ObjectId) => (companyId instanceof Types.ObjectId ? companyId.toString() : companyId);
...
const buildChunks = (text: string, chunkSize = 900) => {
...
const makeLeadPayload = (customerProfile: Record<string, any>, companyId: string, conversationId?: string) => ({
...
const buildDashboardContext = (companyId: string, question: string, leads: any[], sales: any[], employees: any[]) => {
...
const buildQuickAnswer = (question: string, leads: any[], sales: any[], employees: any[]) => {
  const normalizedQuestion = question.toLowerCase();

  const includesAny = (...patterns: string[]) => patterns.some((pattern) => normalizedQuestion.includes(pattern));
...
const buildEntityMatchAnswer = (question: string, leads: any[], sales: any[], employees: any[]) => {
  const normalizedQuestion = question.toLowerCase();
  const lookupTargets = [
...
export class AiAutomationService {
  static async getSettings(companyId: string) {
    const existing = await AiSettings.findOne({ companyId });
    if (existing) return existing.toObject();

    const created = await AiSettings.create({
      companyId,
  ...
  static async updateSettings(companyId: string, payload: Record<string, any>) {
    const settings = await AiSettings.findOne({ companyId });
    if (!settings) {
  ...
  static async listConnections(companyId: string) {
    const connections = await AiConnection.find({ companyId }).sort({ createdAt: -1 }).lean();
    return connections.map((connection) => ({
      ...connection,
      apiToken: connection.apiTokenEncrypted ? decryptSecret(connection.apiTokenEncrypted) : '',
      webhookSecret: connection.webhookSecretEncrypted ? decryptSecret(connection.webhookSecretEncrypted) : '',
    }));
  }

  static async createConnection(companyId: string, payload: SaveConnectionPayload) {
    const cleaned: Record<string, any> = {
      ...
      campaignId: payload.campaignId ? new Types.ObjectId(payload.campaignId) : undefined,
      salesGroupId: payload.salesGroupId ? new Types.ObjectId(payload.salesGroupId) : undefined,
    ...
    if (payload.apiToken) {
      cleaned.apiTokenEncrypted = encryptSecret(payload.apiToken);
    }

    if (payload.webhookSecret) {
      cleaned.webhookSecretEncrypted = encryptSecret(payload.webhookSecret);
    }

    const connection = await AiConnection.create(cleaned);
    return connection.toObject();
  }

  static async updateConnection(companyId: string, connectionId: string, payload: Partial<SaveConnectionPayload>) {
    const connection = await AiConnection.findOne({ companyId, _id: connectionId });
    if (!connection) {
      throw { statusCode: 404, message: 'Connection not found.' };
    }

    if (payload.name !== undefined) connection.name = normalizeString(payload.name) || connection.name;
    if (payload.siteId !== undefined) connection.siteId = normalizeString(payload.siteId) || undefined;
    if (payload.channelId !== undefined) connection.channelId = normalizeString(payload.channelId) || undefined;
    if (payload.providerId !== undefined) connection.providerId = normalizeString(payload.providerId) || undefined;
    if (payload.apiEndpoint !== undefined) connection.apiEndpoint = normalizeString(payload.apiEndpoint) || undefined;
    if (payload.connectedCampaign !== undefined) connection.connectedCampaign = normalizeString(payload.connectedCampaign) || undefined;
    if (payload.campaignId !== undefined) connection.campaignId = payload.campaignId ? new Types.ObjectId(payload.campaignId) : undefined;
    if (payload.salesGroupId !== undefined) connection.salesGroupId = payload.salesGroupId ? new Types.ObjectId(payload.salesGroupId) : undefined;
    if (payload.isActive !== undefined) connection.isActive = payload.isActive;
    if (payload.webhookStatus !== undefined) connection.webhookStatus = payload.webhookStatus;

    if (payload.apiToken) {
      connection.apiTokenEncrypted = encryptSecret(payload.apiToken);
    }

    if (payload.webhookSecret) {
      connection.webhookSecretEncrypted = encryptSecret(payload.webhookSecret);
    }
  ...
  static async validateConnection(companyId: string, connectionId: string) {
    const connection = await AiConnection.findOne({ companyId, _id: connectionId });
    if (!connection) {
      throw { statusCode: 404, message: 'Connection not found.' };
    }
    ...
    if (!connection.apiEndpoint) validationNotes.push('API endpoint is missing.');
    if (!connection.apiTokenEncrypted) validationNotes.push('API token is missing.');
    if (!connection.siteId) validationNotes.push('Site ID is missing.');
    if (!connection.channelId) validationNotes.push('Channel ID is missing.');
  ...
  static async listKnowledgeBases(companyId: string) {
    return AiKnowledgeBase.find({ companyId }).sort({ createdAt: -1 }).lean();
  }

  static async createKnowledgeBase(companyId: string, payload: { name: string; description?: string; connectedCampaign?: string; campaignId?: string; isActive?: boolean }) {
    const name = normalizeString(payload.name);
    if (!name) throw { statusCode: 400, message: 'Knowledge base name is required.' };

    const knowledgeBase = await AiKnowledgeBase.create({
      companyId,
      name,
  ...
  static async uploadKnowledgeFile(companyId: string, knowledgeBaseId: string, file: { buffer: Buffer; originalname: string; mimetype: string }) {
    const knowledgeBase = await AiKnowledgeBase.findOne({ companyId, _id: knowledgeBaseId });
    if (!knowledgeBase) {
    ...
    await AiKnowledgeChunk.deleteMany({ companyId, knowledgeBaseId });
    ...
    await AiKnowledgeChunk.insertMany(embeddings);
  ...
  static async searchKnowledgeBase(companyId: string, knowledgeBaseId: string, userMessage: string, options?: { limit?: number; threshold?: number }) {
    const kb = await AiKnowledgeBase.findOne({ companyId, _id: knowledgeBaseId, isActive: true });
    if (!kb) {
      throw { statusCode: 404, message: 'Knowledge base not found.' };
    }
    ...
    const chunks = await AiKnowledgeChunk.find({ companyId, knowledgeBaseId }).lean();
    if (!chunks.length) {
      return [];
    }
  ...
  static async listConversations(companyId: string) {
    return AiConversation.find({ companyId }).sort({ createdAt: -1 }).lean();
  }

  static async getConversation(companyId: string, conversationId: string) {
    const conversation = await AiConversation.findOne({ companyId, _id: conversationId }).lean();
    if (!conversation) {
  ...
  static async getDashboard(companyId: string) {
    const [connections, knowledgeBases, conversations, pendingLeads] = await Promise.all([
      AiConnection.countDocuments({ companyId }),
      AiKnowledgeBase.countDocuments({ companyId }),
      AiConversation.countDocuments({ companyId }),
      AiConversation.countDocuments({ companyId, leadReady: true }),
    ]);

    const activeChats = await AiConversation.countDocuments({ companyId, status: 'AI_ACTIVE' });
    const humanHandoffs = await AiConversation.countDocuments({ companyId, status: 'AI_TO_HUMAN' });
  ...
  static async askCompanyData(companyId: string, question: string) {
    const settings = await this.getSettings(companyId);

    const [leads, sales, employees] = await Promise.all([
      Lead.find({ companyId }).sort({ createdAt: -1 }).lean(),
      Sale.find({ companyId }).sort({ saleDate: -1, createdAt: -1 }).lean(),
      Employee.find({ companyId }).select('name role email').sort({ createdAt: -1 }).lean(),
    ]);
    ...
    try {
      const answer = await AiService.askLocalModel(systemPrompt, question, [context], {
        model: settings.activeModel,
  ...
  static async processIncomingJivoEvent(companyId: string, payload: JivoWebhookPayload) {
    ...
    const knowledgeBase = connection.campaignId ? await AiKnowledgeBase.findOne({ companyId, _id: connection.campaignId, isActive: true }) : null;
    ...
    const structuredReply = await AiService.generateStructuredReply({
      systemPrompt,
      conversationContext,
    ...
    if (leadReady && !conversation.leadId) {
      ...
      const createdLead = await CompanySalesService.createLead(companyId, leadPayload);
      conversation.leadId = createdLead._id as Types.ObjectId;
  ...
  private static async resolveConnection(companyId: string, payload: JivoWebhookPayload) {
    const connectionFilters: any[] = [{ companyId, isActive: true }];

    const identifiers = [payload.connectionId, payload.chat?.id, payload.siteId, payload.channelId].filter(Boolean);

    if (payload.connectionId) {
      const direct = await AiConnection.findOne({ companyId, _id: payload.connectionId, isActive: true });
    ...
    const connection = await AiConnection.findOne({
  ...
  private static extractIncomingMessage(payload: JivoWebhookPayload) {
    if (payload.message?.content) return normalizeString(payload.message.content);
    if (payload.message?.text) return normalizeString(payload.message.text);
    if (payload.chat?.lastMessage) return normalizeString(payload.chat.lastMessage);
  ...
  private static async getOrCreateConversation(companyId: string, connection: any, payload: JivoWebhookPayload) {
    const jivoChatId = payload.chat?.id || payload.chatId || payload.id || '';
    const customerId = payload.customer?.id || payload.customerId || '';

    let conversation = await AiConversation.findOne({ companyId, jivoChatId });
    ...
    if (payload.chat?.status) {
      if (payload.chat.status === 'closed') conversation.status = 'CLOSED';
    }
  ...
  private static mergeCustomerProfile(existingProfile: Record<string, any>, payload: JivoWebhookPayload) {
    const incoming = {
      ...(payload.customer || {}),
  ...
  private static shouldCreateLead(customerProfile: Record<string, any>, settings: any) {
    const far = settings.leadCompletionRules?.requiredFields || ['name', 'country', 'mobile', 'email', 'system', 'issue'];
    const requiredFields = far.map((field: string) => {
      if (field === 'mobile') return customerProfile.mobile || customerProfile.phone;
      if (field === 'email') return customerProfile.email;
  ...
  private static shouldHandoff(
    customerProfile: Record<string, any>,
    structuredReply: { confidence?: number; handoff_required?: boolean },
    settings: any,
    knowledgeResults: KnowledgeSearchResult[]
  ) {
    if (structuredReply.handoff_required) return true;
    ...
    if (knowledgeResults.length === 0 && score < threshold) return true;
    ...
    if (angryKeywords.some((keyword) => text.toLowerCase().includes(keyword))) return true;

    return false;
  }

  private static async notifyCampaignGroup(companyId: string, connection: any, lead: any) {
    if (!connection.salesGroupId) return;

    const group = await Group.findOne({ companyId, _id: connection.salesGroupId });
    if (!group) return;

    const recipients = Array.from(new Set([group.createdBy.toString(), ...group.members.map((member: any) => member.toString())]));
  ...
  private static async notifyHandoff(companyId: string, connection: any, conversation: any, customerProfile: Record<string, any>) {
    if (!connection.salesGroupId) return;
  ...
  static async verifyWebhookSignature(body: any, signature?: string, secret?: string) {
    const secretValue = secret || process.env.JIVO_WEBHOOK_SECRET || '';
    if (!signature || !secretValue) return true;

    const expected = crypto.createHmac('sha256', secretValue).update(JSON.stringify(body)).digest('hex');
